 ...
 */
inline RegInode::~RegInode() {
    for (auto it = extents_.begin(); it != extents_.end(); it++)
        fs_->free_space(&it->second);
    extents_.clear();
}

inline bool Inode::is_regular() const { return i_st.st_mode & S_IFREG; }
inline bool Inode::is_directory() const { return i_st.st_mode & S_IFDIR; }
inline bool Inode::is_symlink() const { return i_st.st_mode & S_IFLNK; }
